...
/**
 * `checkVisibility` for the Lead traceability section.
 *
 ...
 *
 * `isDerived` rather than `_class === Lead`, so a deployment that derives its own
 * tag from Lead keeps the section.
 *
 ...
 * @public
 */
export async function checkLeadTraceLinksVisibility (doc: Card): Promise<boolean> {
  return getClient().getHierarchy().isDerived(doc._class, crmLite.masterTag.Lead)
}

/**
 * `checkVisibility` for the Lead required-field checklist.
 *
 * Same predicate as {@link checkLeadTraceLinksVisibility}, and deliberately a
 * SEPARATE resource rather than a shared one: the two sections are independent
 * blocks and either may later grow a condition of its own (the checklist could
 * hide itself once complete, the traceability block could not). Sharing one
 * resource now would make that a breaking change to the other section.
 *
 * @public
 */
export async function checkLeadFieldsVisibility (doc: Card): Promise<boolean> {
  return getClient().getHierarchy().isDerived(doc._class, crmLite.masterTag.Lead)
}
